...
const PRODUCTS = [
  ...
  },
  {
    ...
    featured: false, is_active: true,
    variants: [{ id: "13_4", name: "Pack of 4", label: "Pack of 4", price: 119, salePrice: 119, mrp: 150, stock: 50, inStock: true }],
  },
  {
    ...
    featured: true, is_active: true,
    variants: [{ id: "14_4", name: "Pack of 4", label: "Pack of 4", price: 100, salePrice: 100, mrp: 130, stock: 50, inStock: true }],
  },
  {
    ...
    featured: true, is_active: true,
    variants: [{ id: "15_2", name: "Pack of 2", label: "Pack of 2", price: 109, salePrice: 109, mrp: 140, stock: 50, inStock: true }],
  },
  {
    ...
    featured: false, is_active: true,
    variants: [{ id: "16_1", name: "Single piece", label: "Single piece", price: 129, salePrice: 129, mrp: 160, stock: 45, inStock: true }],
  },
  {
    ...
    featured: true, is_active: true,
    variants: [{ id: "17_1", name: "Single piece", label: "Single piece", price: 209, salePrice: 209, mrp: 260, stock: 40, inStock: true }],
  },
  {
    ...
    featured: true, is_active: true,
    variants: [{ id: "18_1", name: "Single piece", label: "Single piece", price: 209, salePrice: 209, mrp: 260, stock: 40, inStock: true }],
  },
  {
    ...
    featured: true, is_active: true,
    variants: [{ id: "19_220", name: "220ml", label: "220ml", price: 199, salePrice: 199, mrp: 250, stock: 50, inStock: true }],
  },
  {
    ...
    featured: false, is_active: true,
    variants: [{ id: "20_150", name: "150ml", label: "150ml", price: 129, salePrice: 129, mrp: 160, stock: 50, inStock: true }],
  },
];
...
async function seed() {
  ...
  if (checkError) {
    console.error('❌ Failed to check existing products:', checkError.message);
    process.exit(1);
  }

  // Fetch a valid event_id if products_event_id_fkey constraint exists
  let validEventId = null;
  const { data: eventsData } = await supabase.from('events').select('id').limit(1);
  if (eventsData && eventsData.length > 0) {
    validEventId = eventsData[0].id;
    console.log(`📌 Found existing event_id: ${validEventId}`);
  } else {
    // Try creating a default event so foreign key passes
    const { data: newEvent, error: newEventErr } = await supabase
      .from('events')
      .insert({ title: 'General Store Event', name: 'General Store Event' })
      .select('id')
      .single();
    if (!newEventErr && newEvent) {
      validEventId = newEvent.id;
      console.log(`📌 Created default event_id: ${validEventId}`);
    } else {
      // Try finding event_id from products
      const { data: prodWithEvent } = await supabase.from('products').select('event_id').not('event_id', 'is', null).limit(1);
      if (prodWithEvent && prodWithEvent.length > 0) {
        validEventId = prodWithEvent[0].event_id;
        console.log(`📌 Found event_id from existing product: ${validEventId}`);
      }
    }
  }

  const existingNames = new Set((existing || []).map(p => p.name || p.title));
  const toInsert = PRODUCTS.filter(p => !existingNames.has(p.name)).map(p => {
    const item = {
      ...p,
      title: p.name,
      name: p.name,
    ...
    if (validEventId) {
      item.event_id = validEventId;
    }
    return item;
  });

  if (toInsert.length === 0) {
    console.log('✅ All 20 products already exist in Supabase. Nothing to insert.');
    return;
  }
